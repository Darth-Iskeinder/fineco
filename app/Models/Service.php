...
class Service extends Model
{
    ...
    public const SPECIAL_FLAGS = [
        ...
        'is_kkm'                 => ['label' => 'ККМ / Касса',         'client' => 'has_kkm',                 'color' => 'blue'],
        'is_alcohol'             => ['label' => 'Алкоголь',            'client' => 'has_alcohol',             'color' => 'rose'],
    ...
    /**
     * Типы обслуживания. Одному БП соответствует не больше одного типа.
     *
     * Пусто — общий БП: участвует при любом обслуживании, как было до появления
     * поля. Отсекается БП только тогда, когда тип у него проставлен, а у клиента
     * этот тип не отмечен. «Полное обслуживание» отдельным типом не заводится:
     * это все три отметки у клиента сразу.
     */
    ...
    /** Название типа обслуживания для показа. */
    public static function serviceTypeLabel(?string $type): ?string
    {
        return $type === null ? null : (self::SERVICE_TYPES[$type] ?? $type);
    }
     ...
     * MANDATORY_CATEGORIES — «обязательные»: подтягиваются всем клиентам, кроме
     *   нулёвки (is_zero_movement), даже без совпадения РН.
     * Прочие категории (напр. «Условная по профилю») особого режима не имеют —
     *   подтягиваются по совпадению РН или по особым условиям, как обычные БП.
     */
