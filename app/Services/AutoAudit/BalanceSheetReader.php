...
class BalanceSheetReader
{
    ...
    public function turnover(string $path, string $account, string $side = 'credit'): DocumentValue
    {
        ...
        if (!$period) {
            // Годовая ведомость вместо месячной: период понятен, просто он не тот. Так и
            // пишем, иначе человек читает «не разобрали» и думает, что сломалась система.
            // На бою 24.09.2026 к сентябрьской задаче приложили ведомость за 2024 год.
            if (preg_match('/ведомость\s+за\s+(\d{4})\s*г/u', $head, $m)) {
                return DocumentValue::wrongDocument("Ведомость за {$m[1]} год, а нужна за месяц");
            }
