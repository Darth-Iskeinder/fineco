...
final class ClientImportPlanner
{
    ...
    private function planRow(array $values, array $seenInn): array
    {
        ...
        foreach ($this->references() as $key => [$book, $field, $title]) {
            ...
            if ($id === null) {
                // Мягкий справочник: значение просто не подставляем, строка живёт дальше.
                if (in_array($key, self::SOFT_REFERENCES, true)) {
                    continue;
                }
    ...
    /**
     * Справочники, из-за которых строку не отбиваем.
     *
     * Вид деятельности в системе ни на что не влияет: он только показывается в
     * карточке, ни подтягивание БП, ни задачи, ни цена его не спрашивают. Своё
     * название из чужой таблицы («Розница», «торговля розничная») стоило целой
     * строки клиента — это дороже самой ошибки. Незнакомое значение пропускаем:
     * клиент создаётся с пустым полем, у существующего прежнее остаётся как было.
     *
     * Остальные справочники строгие намеренно. Режим налогообложения решает,
     * какие БП подтянутся в смету, статус закрывает обслуживание, ответственный
     * решает, кто клиента видит: там пустое поле это тихая ошибка, которую
     * заметят через месяц по отсутствию задач.
     */
    private const SOFT_REFERENCES = ['activity_type'];
