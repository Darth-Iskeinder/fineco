 ...
 *  - дочернюю рождает только настоящее выполнение: у принудительно закрытой
 *    задачи триггер молчит;
...
class EventTriggeredTasks
{
    ...
    private static function spawn(BuhTaskLog|BuhAdhocTask $task): ?BuhAdhocTask
    {
        ...
        // Принудительное закрытие — не выполнение: человек записал причину, почему
        // работы не будет. Значит и следующей работы нет. Так же на такие задачи
        // смотрит автоаудит: документа он с них не ждёт.
        //
        // Отметка живёт на задаче до самой приёмки, поэтому она гасит триггер и на
        // «Выполнено», и позже, когда главбух нажмёт «Принять». Снимает её обычное
        // закрытие (после доработки) и сброс задачи — тогда триггер снова в силе.
        // У внеплановых задач принудительного закрытия нет вовсе.
        if ($task instanceof BuhTaskLog && $task->force_closed) {
            return null;
        }
