...
<div x-data="tariffsPage()" class="space-y-4">

    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/50 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-semibold text-slate-800">Тарифы</h2>
                <p class="text-sm text-slate-500 mt-0.5">Тарифные планы для клиентов</p>
            </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="bg-white divide-y divide-slate-200">
                    ...
                    <template x-if="items.length === 0">
                        <tr><td colspan="4" class="px-6 py-10 text-center text-slate-400">Нет тарифов</td></tr>
                    </template>
                </tbody>
            </table>
        </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl p-6 z-10">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-lg font-semibold text-slate-900" x-text="isEditing ? 'Редактирование тарифа' : 'Новый тариф'"></h3>
                    ...
                    <form @submit.prevent="submit()">
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea x-model="form.description" rows="3" class="block w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 resize-none"></textarea>
                            </div>
                            <div>
...
function tariffsPage() {
    return {
        ...
        openEdit(item) {
            ...
            this.form = { name: item.name, description: item.description || '', is_active: item.is_active };
            this.showModal = true;
        },
        openDelete(item) { this.deleteItem = item; this.showDeleteModal = true; },
        closeModal() { this.showModal = false; this.resetForm(); },
        resetForm() { this.form = { name: '', description: '', is_active: true }; },

        async submit() {
            this.saving = true;
            const url = this.isEditing ? `/settings/tariffs/${this.editingId}` : '/settings/tariffs';
            try {
        ...
        async confirmDelete() {
            this.deleting = true;
            try {
                const r = await fetch(`/settings/tariffs/${this.deleteItem.id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                const d = await r.json();
