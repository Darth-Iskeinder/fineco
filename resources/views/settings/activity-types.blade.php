...
@section('page-title', 'Виды деятельности')

@section('settings-content')
<div x-data="activityTypesPage()" class="space-y-4">

    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/50 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-semibold text-slate-800">Виды деятельности</h2>
                <p class="text-sm text-slate-500 mt-0.5">Классификатор видов деятельности клиентов</p>
            </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl p-6 z-10">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-lg font-semibold text-slate-900" x-text="isEditing ? 'Редактирование' : 'Новый вид деятельности'"></h3>
...
function activityTypesPage() {
    return {
        ...
        async submit() {
            this.saving = true;
            const url = this.isEditing ? `/settings/activity-types/${this.editingId}` : '/settings/activity-types';
            try {
        ...
        async confirmDelete() {
            this.deleting = true;
            try {
                const r = await fetch(`/settings/activity-types/${this.deleteItem.id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                const d = await r.json();
