        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">{{ $nameLabel ?? 'Название' }}</th>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl p-6 z-10">
                    ...
                    <form @submit.prevent="submit()">
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">{{ $nameLabel ?? 'Название' }} <span class="text-red-500">*</span></label>
                            ...
                            <template x-for="f in fields" :key="f.key">
                                <div>
                                    ...
                                    <template x-if="f.type === 'text'">
                                        <input type="text" x-model="form[f.key]" :required="f.required"
                                            class="block w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
                                    </template>
                                    <template x-if="f.type !== 'text'">
