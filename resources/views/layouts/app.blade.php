                ...
                <div class="flex items-center space-x-4">
                    @auth('employee')
                        {{-- Помощь. Кнопка рядом с профилем: справка лежит в отдельном
                             ...
                             Пункт пока один, меню заведено сразу под несколько. --}}
                        <div class="relative" x-data="{ helpOpen: false }">
                            <button @click="helpOpen = !helpOpen"
                                    type="button"
                                    title="Нужна помощь?"
                                    class="w-9 h-9 flex items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                                    :class="{ 'bg-slate-100 text-slate-600': helpOpen }">
                            ...
                            <div x-show="helpOpen"
                                 ...
                                 style="display: none;">
                                <p class="px-4 pt-1 pb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Нужна помощь?</p>
                                <a href="{{ route('docs.index') }}"
                                   target="_blank"
                                   rel="noopener"
                                   @click="helpOpen = false"
                                   class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors duration-150">
    ...
    <script>
        function taskAlerts() {
            return {
                ...
                fmt(date) {
                    // Пустая дата приходит у задач без срока. x-show прячет строку, но
                    // x-text Alpine считает всё равно, и без этой проверки карточка
                    // уведомлений падала с ошибкой на первой же такой задаче.
                    if (!date) return '';
